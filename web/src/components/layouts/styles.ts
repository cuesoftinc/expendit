...
const styles = {
  container: (mb?: boolean) =>
    `fixed ${
      mb
        ? "block sm:w-[45%] w-[80%] bg-white rounded-tr-lg rounded-br-lg "
        : "lg:block hidden w-[25%]"
    } h-screen  shadow-xl pl-3`,
  ...
  link: (url: string, ln: string) =>
    `flex gap-5 items-center pl-4 py-3 rounded-lg m-2 my-4 hover:bg-purple-200 ${
      url === ln && "bg-purpleTheme text-white hover:bg-purpleTheme"
    }`,
  ...
  // Navbar Styles -------------
  navCont:
    "flex justify-between p-2 relative bg-white w-full md:pr-6 md:pl-4 pl-0 pr-2",
  center: `${genStyles.flexCenter}`,
  searchIcon: "translate-x-6 cursor-pointer dark:text-gray-200",
  navInput:
    "outline-none bg-grayTheme pl-8 px-3 py-2 rounded-md md:text-base text-sm",
  navBtn:
    "md:flex items-center gap-3 rounded-md p-2 px-3 bg-grayTheme hover:opacity-80 shadow-sm hidden",
  profileCont:
    "flex items-center p-1 rounded-full bg-grayTheme hover:opacity-80 gap-2 cursor-pointer",
  ...
  // UserProfile Styles --------
  userProfileCont:
    "nav-item absolute right-1 top-16 bg-white p-8 rounded-lg w-96 z-10",
  ...
  barCont:
    "flex gap-5 border-b-1 border-color p-4 hover:bg-grayTheme cursor-pointer",
  profileLogout:
    "rounded-md py-4 bg-purpleTheme text-center w-full text-white hover:opacity-90",
};
