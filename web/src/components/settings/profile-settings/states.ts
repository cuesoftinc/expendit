...
import { userDetailsApi } from "@/api/apis/user-api";
import {
  formatUserDetails,
  userDetailsRequiredFields,
} from "@/utils/format-user-details";
import { UserDetailsPayload } from "@/api/types";
import { useHomeContext } from "@/context";

export const useDetailsCustomState = () => {
  const { setFormError, setFormSuccess, setFormLoading, setUser, formLoading } =
    useHomeContext();
