...
import { passwordChangeApi } from "@/api/apis/auth-api";
import {
  formatPasswordChange,
  passwordRequiredFields,
} from "@/utils/format-password-change";
import { PasswordPayload } from "@/api/types";
import { useHomeContext } from "@/context";

export const usePasswordCustomState = () => {
  const { formLoading, setFormLoading, setFormError, setFormSuccess } =
    useHomeContext();
