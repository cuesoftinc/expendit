...
import { getCategoryApi } from "../api/apis/category-api";
import { getExpenseApi, getMonthlyExpenseApi } from "../api/apis/expense-api";
...
export const HomeProvider = ({ children }: HomeProviderProps) => {
  ...
  useEffect(() => {
    ...
    async function getInitialReportsData() {
      try {
        if (user !== null && isMounted) {
          const [areaChartRes, barChartRes, pieChartRes, lineChartRes] =
            await Promise.all([
              getAreaHomeChartApi(),
              getBarChartApi(),
              getPieChartApi(),
