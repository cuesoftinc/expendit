...
type ImportResult struct {
	...
	Transactions []models.ImportedTransaction  `json:"transactions"`
}
...
func ProcessImport(ctx context.Context, userID, fileName string, data []byte) (*ImportResult, error) {
	...
	var staged []models.ImportedTransaction
	duplicateCount := 0
	...
	for _, raw := range rawTxns {
		...
		staged = append(staged, models.ImportedTransaction{
			ID:          primitive.NewObjectID(),
			ImportJobID: job.ID,
			UserID:      userID,
			Date:        raw.Date,
...
func ConfirmImport(ctx context.Context, jobID primitive.ObjectID, userID string) error {
	...
	var txns []models.ImportedTransaction
...
func GetImportJob(ctx context.Context, jobID primitive.ObjectID, userID string) (*models.ImportJob, []models.ImportedTransaction, error) {
...
func generateImportSummary(ai *AIEnhancer, staged []models.ImportedTransaction, summary models.ImportSummary) string {
...
func markJobFailed(ctx context.Context, jobID primitive.ObjectID, err error) error {
	importJobCol.UpdateOne(ctx,
		bson.M{"_id": jobID},
		bson.M{"$set": bson.M{"status": models.ImportStatusFailed, "error": err.Error()}},
	)
	return err
}
