...
func DetectAnomalies(ctx context.Context, expCol *mongo.Collection, userID string, txns []models.ImportedTransaction) []models.Anomaly {
	...
	for _, txn := range txns {
		...
		if avgAmount > 0 && txn.Amount > avgAmount*3 {
			anomalies = append(anomalies, models.Anomaly{
				Type:        models.AnomalyLargeTransaction,
	...
	for category, total := range importTotals {
		...
		key = string(models.AnomalyAbnormalCategory) + ":" + category
		if avg, ok := threeMonthAvg[category]; ok && avg > 0 && !seen[key] {
			pct := (total - avg) / avg * 100
			if pct > 50 {
				seen[key] = true
				anomalies = append(anomalies, models.Anomaly{
					Type:        models.AnomalyAbnormalCategory,
