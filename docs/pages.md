...
| A8a | Self-host **[Directive 2026-07-18]** | data-ownership pitch ("your ledger, your infra"); tabbed snippet **Docker Compose \| Helm** (CodeSnippet `tabs=docker/helm`): mirrored two-line commands — `git clone https://github.com/cuesoftinc/expendit` then `cd expendit && docker compose up --build -d` (Makefile `up` target) vs `cd expendit && helm install expendit deploy/helm` — only the final command differs, with a muted caption under the block ("Compose ships MongoDB + Redis — the Helm chart expects reachable instances (MONGODB_URL, REDIS_URL)") **[Ratified 2026-07-20]**; what ships (api-common · web · Mongo · Redis, architecture.md); self-host docs link | copy ✓ morph |
